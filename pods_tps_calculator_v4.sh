...
show_top_memory_usage() {
  echo
  echo "==================== 💾 TOP POD MEMORY USAGE ===================="
  kubectl top pod --all-namespaces --sort-by=memory \
    | head -n 15 \
    | awk '{printf "%-18s %-40s %-12s %-12s\n", $1,$2,$3,$4}'

  echo
  echo "==================== 💾 TOP DEPLOYMENT MEMORY USAGE ===================="
  kubectl top pod --all-namespaces \
    | tail -n +2 \
    | awk '
        {
          # extract deployment name from pod name if it matches deployment pattern
          split($2, a, "-")
          dep=a[1]
          for(i=2;i<=length(a)-2;i++){dep=dep"-"a[i]}
          mem=$4
          # convert to Mi
          if(mem ~ /Ki$/) mem=mem*0.0009765625
        ...
        END {
          for(d in usage) printf "%-18s %-40s %10.0f Mi\n", ns[d], d, usage[d]
        }
      ' | sort -k3 -nr | head -n 15

 
echo
echo "==================== 💾 TOP HPA MEMORY UTILIZATION ===================="
printf "%-18s %-90s %-10s %-12s %-4s %-4s %-4s\n" \
  "NAMESPACE" "HPA NAME" "CPU(%)" "MEM(Mi)" "MIN" "CUR" "MAX"
printf "%-18s %-90s %-10s %-12s %-4s %-4s %-4s\n" \
  "------------------" \
  "------------------------------------------------------------------------------------------" \
  "----------" "------------" "----" "----" "----"
...
| jq -r --argjson now "$NOW_EPOCH" '
    ...
    | [
        ...
        (
          ( .status.currentMetrics // []
            | map(select(.resource.name=="cpu") | (.resource.current.averageUtilization // "n/a"))
        ...
        (
          ( .status.currentMetrics // []
            | map(
                (
                  (select(.resource.name=="memory")?.resource.current.averageValue?) //
                  (select(.containerResource.name=="memory")?.containerResource.current.averageValue?)
                )
                | select(type=="string")
                | capture("(?<num>[0-9]+)(?<unit>.*)") as $m
                | (
                    if   $m.unit=="Mi" then ($m.num|tonumber)
                    elif $m.unit=="Ki" then (($m.num|tonumber)/1024)
                    elif $m.unit=="Gi" then (($m.num|tonumber)*1024)
                    else (($m.num|tonumber)/1048576)
                    end
                  | floor
                  | tostring
                )
    ...
    | @tsv
' | sort -k4 -nr \
  | awk '{printf "%-18s %-90s %-10s %-12s %-4s %-4s %-4s\n", $1,$2,$3,$4,$5,$6,$7}'
}
...
if [[ "${1:-}" == "--cluster-assess" ]]; then
  ...
  compute_tps
  cluster_assessment
  show_top_memory_usage    # ✅ match the actual function name
  show_cpu_provisioners
else
