...
show_hpa_status() {
  ...
  printf "%-18s %-90s %-10s %-4s %-4s %-4s %-6s\n" \
  ...
  | jq -r --argjson now "$NOW_EPOCH" '
      ...
      | [
          ...
          (
            if (.metadata.creationTimestamp? | type)=="string" then
              ((.metadata.creationTimestamp | fromdate?) as $ts
               | if ($ts|type)=="number"
                 then (( $now - $ts ) / 86400 | floor | tostring + "d")
                 else "n/a"
                 end)
            else "n/a"
            end
